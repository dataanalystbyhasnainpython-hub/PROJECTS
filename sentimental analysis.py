...
def analyze_sentiment_vader(text):
    """Analyze sentiment using TextBlob (VADER is optional)"""
    blob = TextBlob(text)
    polarity = blob.sentiment.polarity
    subjectivity = blob.sentiment.subjectivity

    # Classify sentiment with custom thresholds
    if polarity > 0.1:
        sentiment = "Positive"
        emoji = "😊"
...
def get_reaction_badge(sentiment):
    """Get reaction badge HTML"""
    if sentiment == "Positive":
        return '<span style="font-size: 24px;">😊</span> Positive'
    elif sentiment == "Negative":
        return '<span style="font-size: 24px;">😞</span> Negative'
    else:
        return '<span style="font-size: 24px;">😐</span> Neutral'


# Main Header
st.markdown(
    '<div class="main-header"><h1>🎯 Customer Feedback Sentiment Analysis</h1><p>AI-Powered Real-Time Feedback Classification System</p></div>',
    unsafe_allow_html=True)
...
if analysis_mode == "🎯 Single Analysis":
        ...
        if st.button("🔍 Analyze Sentiment", type="primary", use_container_width=True):
            if feedback_text.strip():
                with st.spinner("Analyzing sentiment..."):
                    # Perform analysis
                    result = analyze_sentiment_vader(feedback_text)
                    ...
                    with res_col1:
                        sentiment_color = "green" if result['sentiment'] == "Positive" else "red" if result[
                                                                                                         'sentiment'] == "Negative" else "orange"
                        st.markdown(f"""
                            <div class="metric-card" style="background-color: {'#d4edda' if result['sentiment'] == 'Positive' else '#f8d7da' if result['sentiment'] == 'Negative' else '#fff3cd'}">
                                <h3>Sentiment</h3>
    ...
    if st.session_state.feedback_history:
        ...
        if 'customer' in filtered_df.columns and customer_filter != 'All':
